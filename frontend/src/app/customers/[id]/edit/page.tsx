...
export default function EditCustomerPage() {
  ...
  useEffect(() => {
    const fetchCustomer = async () => {
      try {
        ...
        setValue('openingBalance', Number(c.openingBalance));
        setLoading(false);
      } catch (err: any) {
  ...
  const onSubmit = async (values: CustomerFormValues) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to update customer ledger.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6 font-sans text-xs">
      ...
      <div className="flex items-center space-x-3 border-b border-slate-800 pb-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">Edit Customer Ledger</h1>
