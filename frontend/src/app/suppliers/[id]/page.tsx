...
export default function SupplierDetailPage() {
  ...
  if (loading) {
    return <div className="text-center py-20 text-xs text-slate-400">Loading supplier ledger...</div>;
  }

  if (error || !supplier) {
    return (
      <div className="max-w-4xl mx-auto space-y-4 font-sans text-xs">
        <Link href="/suppliers" className="inline-flex items-center space-x-1 text-indigo-400 hover:underline">
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 font-sans text-xs">
      ...
      <div className="flex items-center space-x-3 border-b border-slate-800 pb-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">{supplier.name}</h1>
        ...
        <div className="card-panel space-y-2 bg-slate-900 md:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-slate-300">
            <div className="flex items-center space-x-2">
              ...
              <span>Contact: {supplier.contact || 'N/A'}</span>
            </div>
            <div className="flex items-start space-x-2">
              <MapPin className="w-3.5 h-3.5 text-slate-500 shrink-0 mt-0.5" />
              <span>Address: {supplier.address || 'No Address Specified'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="card-panel flex flex-col justify-between bg-slate-900">
          ...
          <div className="mt-3 space-y-1.5">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-slate-200">${supplier.openingBalance.toFixed(2)}</span>
            </div>
            <div className="flex justify-between border-t border-slate-800/80 pt-1.5">
              ...
              <span className={`font-black text-sm ${supplier.outstandingBalance > 0 ? 'text-rose-455' : supplier.outstandingBalance < 0 ? 'text-emerald-400' : 'text-slate-400'}`}>
              ...
              {ledger.map((entry) => (
                <tr key={entry.id} className={entry.type === 'OPENING' ? 'bg-slate-950/40 text-slate-400 italic' : ''}>
                  ...
                  <td className="font-mono font-bold text-slate-300">
                    {entry.type === 'PURCHASE' ? (
                      <Link href={`/purchase-vouchers/${entry.id}`} className="text-amber-400 hover:underline">
                        {entry.refNo}
                      </Link>
                    ) : (
                      entry.refNo
