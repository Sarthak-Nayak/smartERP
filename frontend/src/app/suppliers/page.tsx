...
export default function SuppliersPage() {
  ...
  const fetchSuppliers = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to fetch supplier ledgers.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Are you sure you want to delete Supplier Ledger '${name}'?`)) return;
    try {
      await apiRequest(`/suppliers/${id}`, { method: 'DELETE' });
      fetchSuppliers();
    } catch (err: any) {
      alert(err.message || 'Failed to delete supplier. Ensure they have no active transactions.');
    }
  };

  const handleOpenPayment = (supplier: Supplier) => {
    setSelectedSupp(supplier);
    setPayAmount('');
    setPayNote('');
    setPaymentOpen(true);
  };
  ...
  const handleRecordPayment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedSupp || !payAmount || Number(payAmount) <= 0) return;

    setPaymentSubmitting(true);
    try {
      await apiRequest(`/suppliers/${selectedSupp.id}/payments`, {
        method: 'POST',
        body: JSON.stringify({
  ...
  const columns = useMemo<ColumnDef<Supplier>[]>(() => [
    {
      ...
      cell: ({ row }) => (
        <Link 
          ...
          className="text-amber-450 hover:text-amber-400 font-bold flex items-center hover:underline"
        >
          {row.original.name}
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return (
          <span className={`font-black ${val > 0 ? 'text-rose-450' : val < 0 ? 'text-emerald-405' : 'text-slate-400'}`}>
            {val.toFixed(2)}
          </span>
        );
      }
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-800 pb-4">
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">Supplier Ledgers</h1>
          <p className="text-xs text-slate-400 mt-1">Manage vendor ledgers, record outgoing disbursements, and monitor active outstanding balances</p>
        </div>
        <Link
          href="/suppliers/new"
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border border-dashed border-slate-800 rounded-lg text-slate-500 text-xs">
          No supplier records found. Click &ldquo;New Supplier&rdquo; or press `Alt+S` to add.
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center px-4">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-sm rounded-xl shadow-2xl overflow-hidden">
            <div className="bg-slate-950 px-4 py-3 border-b border-slate-700 flex justify-between items-center">
              <h3 className="text-xs font-bold text-white uppercase tracking-wider">Record Payment (Disbursement)</h3>
            ...
            <form onSubmit={handleRecordPayment} className="p-4 space-y-4 font-sans text-xs">
              <div>
                ...
                <div className="form-input bg-slate-950 font-bold border-slate-800 text-amber-300">
                  {selectedSupp.name}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="form-input bg-slate-950 text-rose-305 font-bold border-slate-800">
                  ${selectedSupp.outstandingBalance.toFixed(2)}
                </div>
              </div>
              ...
              <div className="flex space-x-2 pt-2">
                ...
                <button
                  ...
                  disabled={paymentSubmitting}
                >
                  {paymentSubmitting ? 'Posting...' : 'Record Disbursement'}
                </button>
              </div>
            </form>
          </div>
